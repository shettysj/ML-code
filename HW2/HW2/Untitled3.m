...
TestingError=[];
 for i=1:9 
   ...
   common = intersect(variable',testingRows(i,:)); 
    ...
    for k=1:240 
        training(k,:,i) = data(trainingRows(i,k),:); 
    end
end
%training =data(1:240,:);
%testing =data(241:300,:);
...
for set=1:9
    ...
    for iterations=1:10 
        ...
        for i=1:240 
            y = x1(i)*w1 + x2(i)*w2 + b; 
            if(y>=1)
                outputTraining(i,1) = 1; 
            else
                outputTraining(i,1) = 0;
            end
        ...
        errorPercentageTraining(iterations,1) = (nnz(outputTraining-training(:,3)))/240; 
    end
%     figure(fig)
...
for i=1:60
    ...
    if(y(i)>=1) 
     ...
     TestingError(end+1)= testing (i,3,set)- outputTesting(i,1);
     check=nnz(TestingError);
end
